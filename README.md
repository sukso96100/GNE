...
세계로 뻗어나가는 K-Programming Language 에 맞게 global 리더십을 갖습니다. 따라서 늘 전역객체(global scope)입니다.
객체를 창조하면 각 모듈 소재의 창조객체혁신센터에 할당됩니다. 실체는 없고 성과만 창조하기 위해서 하나의 객체 당 수십배의 메모리 공간이 필요합니다.
메모리 관리에 신경 써주시길 바랍니다.
...
* `빨간색 마티즈`에 의해 수집당하는 것을 막으려면 객체를 `고양이`로 감싸고 `판사님`을 호출해야 합니다.
```
